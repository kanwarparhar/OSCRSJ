...
## Current State
**Session 16 (2026-04-21, Franklin Cowork) shipped Design System v2.3 "Clinical-Tight" — NEJM/JBJS density pass (Phase 1 of 3).** Body text scale dropped from 16px/1.65 to 15px/1.55 (matches top-tier clinical-journal reading rhythm). `.page-title` reworked to `clamp(30px, 3.4vw, 38px)` + line-height 1.08. `.section-heading` reworked to `clamp(24px, 2.6vw, 30px)` + line-height 1.1. `.card` internal padding 24→20 (p-6 → p-5). Two new utility classes in `globals.css`: `.section-rule` (1px brown/12 top border, clinical-tight alternative to cream-alt color swaps) + `.section-tight` (py-14 px-4 sm:px-6 lg:px-8, canonical internal-page section padding). Homepage masthead hero at `app/page.tsx` tightened: minHeight 520→400, padding 96→64, wordmark clamp(64, 11vw, 144) → clamp(52, 8vw, 96), eyebrow 13→12px, subtitle clamp(20, 2.6vw, 30) → clamp(18, 2.2vw, 24), 5 section padding blocks '72px 24px' → '56px 24px'. `components/PageHeader.tsx` tightened: minHeight 280→220, padding 80→56 — propagates to every internal page. Sweep of `py-20` → `py-14` applied via sed across 30 internal `.tsx` files (articles/*, guide-for-authors, about, editorial-board, submit, apc, faq, peer-review, topics/*, indexing, open-access, accessibility, subscribe, contact, privacy, terms, aims-scope, media, article-types, templates, for-reviewers, for-reviewers/apply, editorial-policies, news, news/ai-in-orthopedics + all [slug] archives). Net vertical-padding reduction per internal page: 60–80px (PageHeader) + 48px top/bottom (section wrapper) ≈ 200px of reclaimed viewport. `tsc --noEmit` clean. Brand identity, palette, type faces, card chrome — all unchanged. Phase 2 (swap body serif) and Phase 3 (palette recalibration) deferred pending review of Phase 1 in production. **No new routes, no new env vars, no new migrations. Route count unchanged at 66.**
