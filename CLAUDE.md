...
- **License:** CC BY-NC-ND 4.0 (locked Session 25). DOAJ + Plan S compliance review with Janine pending.
...
### Session 46 — 2026-05-05 — Franklin Cowork — Guide for Reviewers rebrand + editorial inbox-routing flip + contact-form mailbox-void quick-fix

Three commits on `main`: `2452054` + `d1265fe` + `7d575ed`. **Commit `2452054`** rebranded `/for-reviewers` to "Guide for Reviewers", retired the Reviewer Word-template, added overall-comment + section-by-section line-numbered review structure. URL stays at `/for-reviewers` per Kanwar lock; 8 files +77/-605. **Commit `d1265fe`** consolidated the kanwarparhar@gmail.com → oscrsjournal@gmail.com flip that had been sitting unstaged as 5 orphan files since Session 43, plus 2 more send sites Session 43 missed (`INTERNAL_EDITORIAL_EMAIL` reviewer-pipeline constant + daily-digest fallback ladder). 7 files +26/-21. End-to-end smoke test verified: applicant confirmation arrived at kanwarparhar@gmail.com, internal notification arrived at oscrsjournal@gmail.com. Vercel env-var verification via Chrome MCP revealed Session 43 wrap-up was incorrect — `EMAIL_REPLY_TO` was never set; today's commit is what actually moved Reply-To routing. **Commit `7d575ed`** quick-fixed the contact-form mailbox-void: DNS confirmed `oscrsj.com` apex has NO MX records, so every send to `info@`/`submit@`/`editorial@`/`waivers@oscrsj.com` had been bouncing since Session 27 shipped them ~9 days earlier. `lib/inquiry/actions.ts` flipped `DISCOUNT_INQUIRY_INBOX` + the 5-entry `CONTACT_INBOXES` map to all route to oscrsjournal@gmail.com until Google Workspace is provisioned. **Risks:** code-as-source-of-truth pattern (Manvir refactor candidate — single `lib/email/inboxes.ts`); 9 user-facing `mailto:` links remain on dead-letter @oscrsj.com addresses (deferred). **Kanwar follow-ups:** smoke-test contact form post-deploy; delete the test reviewer application via Supabase Studio; optional bulk mailto: sweep; optional Workspace provisioning. Full narrative + handoffs at [docs/session-history.md#session-46-guide-for-reviewers-rebrand](docs/session-history.md).

### Session 45 — 2026-05-05 — Sushant Cowork — Daily editorial-ops digest cron

Single commit (3 files). Triggered by Kanwar's directive to receive editorial activity notifications without inbox overflow. **Architecture decision (Manvir):** single daily digest, NOT per-event emails — per-event compounds badly at scale. New `app/api/cron/daily-digest/route.ts` runs at `0 13 * * *` daily (staggered an hour earlier than the existing review-reminders + revision-reminders crons), bearer-header gated by `CRON_SECRET`, fires 5 parallel queries over a rolling 24h window (registrations, manuscript submissions, revisions, reviewer applications, reviews submitted), enrichment queries for manuscript titles + author display names, empty-window short-circuit (audit-logs `daily_digest_skipped_empty` and returns without sending). Non-empty windows fire ONE email via `sendEmail()` with `emailType: 'daily_digest'`. New `lib/email/templates/dailyDigest.ts` (~340 lines) using v2 dark-brown shell from Session 30, 5 conditional sections with per-item cards + "Open in admin →" deep links. Subject `[OSCRSJ Daily] N registrations · M submissions · ...`. Recipient ladder: `DIGEST_RECIPIENT_EMAIL` env → `EMAIL_REPLY_TO` env → hardcoded fallback. Third entry in `vercel.json` `crons` array. **Risks:** Vercel Hobby cron quota (3rd cron entry — risk Vercel rejects); first-day digest likely sends empty (audit-log proof-of-life). **§6 separation event:** working tree carried 5 unstaged orphan files from a parallel session (the inbox-routing flip that landed in Session 46). Per §6, all 5 left unstaged. **Kanwar follow-ups:** verify deploy; manual-trigger smoke test via curl; force a non-empty test by signing up a fake user; set up Gmail filter on the `[OSCRSJ Daily]` subject prefix. Full narrative at [docs/session-history.md#session-45-daily-editorial-ops](docs/session-history.md).

### Session 44 — 2026-05-04 — Sushant Cowork — Admin manuscript "Reviewer pool" panel sources from unified roster

Single commit `5bea320` (3 files, +221/-62). Triggered by Kanwar reporting that a freshly-submitted manuscript showed an empty "Active reviewer pool" while the Reviewers tab clearly listed Manvir Kaur in Active status. **Diagnosis via live SQL against Supabase prod:** two surfaces using two different definitions of "active" by construction. The legacy `reviewer_applications.status='active'` column filter returns 0 rows in the entire DB; the Session 32 unified roster's "active" bucket (computed dynamically as `reviewsSubmitted > 0`) returns 1 row (Manvir). Manvir was invited via the Session 11.5 direct-email path which bypasses `reviewer_applications` entirely. **Fix:** `app/dashboard/admin/manuscripts/[id]/page.tsx` swapped from `listReviewerApplications({status:'active'})` to `buildReviewerRoster()` parallel-fetch + side-loaded application data for subspecialty-interest ranking. `InviteReviewerPanel.tsx` rewritten with new `ReviewerPoolEntry` interface, dedupe flipped from applicationId → lowercased email, per-row dispatch on `inviteReviewer` mode (application vs email), bucket pill + reviews-submitted badge. `lib/reviewer/actions.ts` server-side gate relaxed (now allows pending/approved/active applications). Live verification via Chrome MCP confirmed both Manvir (ACTIVE) + Rawnak (APPROVED) now visible. **Risks:** subspecialty match pill regression for direct-email reviewers; `ReviewerPoolEntry` shape duplicates `ReviewerRosterEntry` (mild type-debt); 5 prior-session orphan files left unstaged per §6; Session 43's flagged 3 intervening commits still not backfilled in CLAUDE.md (this refactor session ships the backfill). **Kanwar follow-ups:** E2E test the Invite flow on Manvir's row + Rawnak's row. Full narrative at [docs/session-history.md#session-44-reviewer-pool-unified-roster](docs/session-history.md).
...
- [ ] **Decide whether to push the renderer repo to GitHub** — Sessions 17 + 18 both held local — surfaced Session 18
- [ ] **First manuscript reaches `accepted` → wire payload synthesizer** for renderer — surfaced Session 18
...
- [ ] **Section Editors grid balance** at `sm:grid-cols-2` — odd-count last-row issue with 9 cards (Session 40 expansion) — optional polish
...
- **Editorial board schema:** `lib/schema/editorialBoard.ts`. 17 confirmed members as of Session 40.
