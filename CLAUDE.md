...
- [ ] **Run Migration 020 in Supabase Studio** — `supabase/migrations/020_jats_xml.sql` adds `manuscripts.jats_xml_storage_path TEXT` + `NOTIFY pgrst, 'reload schema'`. Idempotent. Surfaced Session 47.
- [ ] **Run `bash scripts/fetch-jats-dtd.sh`** once on Mac in `~/Documents/oscrsj-renderer/` to pull JATS 1.3 DTD bundle from NCBI. Then capture canonical SHA256 and update `EXPECTED_SHA` constant in the script for future-fetch integrity checks. Surfaced Session 47.
- [ ] **Smoke renderer end-to-end post-DTD-fetch** — `npm run dev` from `~/Documents/oscrsj-renderer/`, exercise `/smoke/sample-payload`, `/smoke/sample-payload-8-authors`, `/smoke/sample-payload-reused-figure`. Confirm JATS validation passes on all 3 (acceptance criteria AC1-AC2 of the spec). Surfaced Session 47.
- [ ] **GitHub closeout for renderer** — from `~/Documents/oscrsj-renderer/` Mac terminal: `rm -f .git/*.lock .git/refs/heads/*.lock && git update-ref refs/heads/main session-20-jats && git checkout main && git branch -d session-20-jats`, then `gh repo create kanwarparhar/oscrsj-renderer --private --source=. --remote=origin --push` (closes Session 18 deferral). Surfaced Session 47.
- [ ] **Correct stale CC BY-NC-ND mention** in CLAUDE.md `## Business Context` — license reverted to CC BY 4.0 on 2026-05-04 (Janine DOAJ + Plan S compliance review) but `## Business Context` still reads "License: CC BY-NC-ND 4.0 (locked Session 25)". One-line edit during a future Manvir wrap. Surfaced Session 47.
