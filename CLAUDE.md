...
| `/dashboard/admin/manuscripts/[id]` | `app/dashboard/admin/manuscripts/[id]/page.tsx` | ✅ Complete (Session 9 shell + Session 11.5 adds `InviteByEmailPanel` + `AdminFileDownloadButton` + Session 12 adds `DecisionComposerPanel` + `DecisionHistoryPanel` sibling cards) |
...
The site is live at oscrsj.com. **Session 13 (2026-04-21, Sushant)** shipped Phase 3.5 cleanups in one commit: migration 011 (columns + enum extensions + partial index + advisory NOTICE on legacy `reject` rows) + new `app/api/cron/revision-reminders/route.ts` (daily 14:00 UTC, mirrors Session 11's review-reminder cron exactly) + new `lib/email/templates/revisionDeadlineReminder.ts` + `vercel.json` second cron entry + new `lib/email/templates/decisionRescindedAuthor.ts` + extended `lib/admin/actions.ts` (`rescindEditorialDecision` server action with editor-ownership + 15-min-window + `deriveRestoredStatus` helper; updated `DECISION_TO_STATUS` mapping for the reject split; `submitEditorialDecision` extended with `reInviteOriginalReviewers` arg + internal `reInviteOriginalReviewers` helper that dedupes by email + idempotent against post-decision rows) + extended `DecisionComposerPanel` (live-countdown Undo banner + Major-Revisions re-invite checkbox + Reject-radio relabelled to "Reject (post-review)" + helper text) + extended `DecisionHistoryPanel` (struck-through rendering for rescinded decisions + reason pill) + extended `lib/types/database.ts` (`post_review_reject` + `rejected` enum members + `rescinded_at/reason` + `revision_reminder_sent_at` columns) + status-pill maps on three list/detail pages updated to include `rejected`. **No new migrations beyond 011, no new env vars (CRON_SECRET reused).** Route count unchanged at 65. Stretch items (batch decisions + revision diff view) deferred to Session 14. Contribution Matrix: `22/23 delivered · 1 blocked` → `26/27 delivered · 1 blocked` (Stripe still LLC-gated). Session 14 priorities in order:

1. **Submission Portal Session 14: Phase 3.5 remainder + end-of-Phase-3 smoke testing** (Sushant Agent scope)
   - 📋 Batch decision actions (stretch from Session 13) — multi-select on `/dashboard/admin/manuscripts` list + bulk decision modal that calls `submitEditorialDecision` sequentially per manuscript with shared letter. Primary use case: desk-reject sweeps after a call for papers.
   - 📋 Revision diff view for editors (stretch from Session 13) — server-side field diff card on the admin manuscript detail page; title / abstract / keywords / author list diff between most recent revision and prior state.
   - ⏳ **Kanwar end-of-Phase-3 smoke test** — single large testing pass covering all 4 decision types on test manuscripts in appropriate statuses → confirm emails land + status transitions correct + audit logs populated. Test rescind by issuing and undoing a decision within 15 min (verify the apology email + status revert). Submit a revision → confirm files land + status flips + emails fire. Wait for cron tick (or `curl -H "Authorization: Bearer $CRON_SECRET" https://www.oscrsj.com/api/cron/revision-reminders`) → confirm revision-deadline reminder fires on a test manuscript with deadline <10 days away. Issue Major Revisions with the re-invite checkbox checked → confirm round-2 invitations seeded + emails fire to the original reviewers.
   - 📋 Reject-enum legacy backfill review — Kanwar checks the migration-011 NOTICE output for any `decision = 'reject'` rows that look like desk-rejects and reclassifies them manually if needed (expected count: 0).
   - 📋 Stripe spec draft if LLC lands. Still blocked otherwise.
   - ⏳ Full end-to-end auth retest — deferred from Sessions 5–13.
   ...
   - ⏳ **Kanwar pre-flight for Session 13's deploy** — run `supabase/migrations/011_session_13_columns.sql` in the Supabase SQL Editor (additive-only; safe to re-run; columns + enum extensions + partial index + advisory NOTICE). No env-var changes; the new revision-reminder cron reuses `CRON_SECRET` provisioned in Session 11.
   - ⏳ Session 11 / 11.5 smoke tests still pending from Session 12 brief. Review decision-letter template copy in the repo (4 strings in `DecisionComposerPanel.tsx` + 1 desk-reject stub); Kanwar can tune wording directly.
   - ⏳ Full end-to-end auth retest — deferred from Sessions 5–12.
   - 📋 Custom auth domain `auth.oscrsj.com` — runbook ready at `docs/supabase-custom-auth-domain.md`.
   - 📋 `EMAIL_REPLY_TO=editorial@oscrsj.com` once Workspace mailbox is provisioned.
